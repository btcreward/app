...
class _ReferralScreenState extends State<ReferralScreen> {
  ...
  void initState() {
    super.initState();
    _loadData();
    _adService.loadRewardedAd(slot: AdSlots.referralRewarded1);
  }
  ...
  Future<void> _shareReferralCode() async {
    ...
    final message =
        'Join me on BTC Reward, a BTC reward simulator. Use my referral code: $referralCode';
    try {
  ...
  Future<void> _claimEarnings() async {
    ...
    final adSuccess = await _adService.showRewardedAd(
      slot: AdSlots.referralRewarded1,
      ...
      onAdDismissed: () {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Please watch the full ad to claim rewards.'),
            backgroundColor: Colors.orange,
          ),
        );
      },
  ...
  // Actual reward claim logic after ad is watched
  Future<void> _claimEarningsAfterAd() async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
            'You can only claim referral rewards once every 24 hours or after 1 AM daily. Do you want to proceed?'),
        actions: [
          TextButton(
      ...
      if (!after1AM && hoursSinceLastClaim < 24) {
        ...
        if (mounted && context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                  'You can only claim referral rewards once every 24 hours or after 1 AM. Next claim: $nextClaimStr'),
              backgroundColor: Colors.orange,
            ),
          );
        }
    ...
    try {
      ...
      if (result['success'] == true) {
        ...
        throw Exception(
            result['message'] ?? 'Failed to claim referral rewards');
      }
    } catch (e) {
      if (!mounted) return;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Failed to claim referral rewards: $errorMsg'),
          backgroundColor: Colors.red,
        ),
      );
    }
    ...
    return Container(
      ...
      child: const Row(
        ...
        children: [
          ...
          Expanded(
            child: Text(
              'Invite friends and collect referral reward points. Eligible referral rewards are added to your pending rewards automatically.',
              style: TextStyle(
                color: Colors.white,
                fontSize: 15,
  ...
  Widget _buildClaimBox() {
    ...
    final canClaim = pendingRewards > 0;
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      '${pendingRewards.toStringAsFixed(18)} BTC points',
                      style: const TextStyle(
                        fontSize: 20,
                        fontWeight: FontWeight.bold,
  ...
  Widget _buildStatItem(String label, String value, IconData icon) {
    // Convert reward values to double and format to 18 decimal places.
    final doubleValue = double.tryParse(value) ?? 0;
    final formattedValue = label.toLowerCase().contains('rewards')
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Reward Balance: $walletBalanceFormatted BTC points',
                  style: const TextStyle(
