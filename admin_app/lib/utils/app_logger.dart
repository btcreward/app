...
/// AdminAppLogger - Logs errors to terminal AND browser console (web)
///
/// Usage:
///   AdminAppLogger.info('Something happened');
///   AdminAppLogger.error('API call failed', error: e, stackTrace: stackTrace);
///   AdminAppLogger.warning('Deprecated API used');
///   AdminAppLogger.debug('Debug info: $data');
class AdminAppLogger {
  // ─── Tags ────────────────────────────────────────
  ...
  static void _log(
    ...
    // 1. Always print → works in terminal + web browser console
    // ignore: avoid_print - Intentional logging framework print statement
    print(fullMessage);

    // 2. Print error details if available
    if (error != null) {
      // ignore: avoid_print - Intentional logging framework print statement
      print('  └─ Error: $error');
    }
  ...
  static void _logToBrowserConsole(
    ...
    try {
      ...
      if (level == _errorTag && error != null) {
        ...
        // ignore: avoid_print - Intentional logging framework print statement
        print('Console Error Object: $error');
        if (stackTrace != null) {
          // ignore: avoid_print - Intentional logging framework print statement
          print('Console StackTrace: ${_formatStackTrace(stackTrace)}');
        }
      }
    } catch (e) {
